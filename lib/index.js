...
function getCondPath(condType = 'in', data = {}) {
  const refNode = t.memberExpression(t.thisExpression(), t.identifier(data.name), data.computed);

  switch (condType) {
    // typeof this.a !== 'undefined' ? this.a : 2;
    case 'typeofUndefined':
      const typeOfThisVal = t.unaryExpression('typeof', refNode, true);
    ...
    case 'hasOwnProperty':
      const hasOwnPropertyExpr = t.unaryExpression('typeof', refNode, true);
